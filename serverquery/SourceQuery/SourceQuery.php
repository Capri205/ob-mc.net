<?php
	...
	class SourceQuery
	{
		...
		public function GetItem($apikey){
			...
			curl_setopt($curl_handle, CURLOPT_URL, self::SERVER_API_ADDRESS.':'.self::SERVER_API_PORT.'/items.json?apikey='.$apikey);
		...
		public function GetStatus(){
			$curl_handle=curl_init();
			curl_setopt($curl_handle, CURLOPT_URL, self::SERVER_API_ADDRESS.':'.self::SERVER_API_PORT.'/status.json');
			curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
			curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($curl_handle, CURLOPT_ENCODING , "gzip");
			$query = curl_exec($curl_handle);
		...
		public function GetConnected($apikey){
			$curl_handle=curl_init();
			curl_setopt($curl_handle, CURLOPT_URL, self::SERVER_API_ADDRESS.':'.self::SERVER_API_PORT.'/players.json?apikey='.$apikey);
			curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
			curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($curl_handle, CURLOPT_ENCODING , "gzip");
			$query = curl_exec($curl_handle);
		...
		public function GetSleepers($apikey){
			$curl_handle=curl_init();
			curl_setopt($curl_handle, CURLOPT_URL, self::SERVER_API_ADDRESS.':'.self::SERVER_API_PORT.'/sleepers.json?apikey='.$apikey);
			curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
			curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($curl_handle, CURLOPT_ENCODING , "gzip");
			$query = curl_exec($curl_handle);
		...
		public function GetRank($limit){
			if($limit != 0)
				$GetPrice = $this->db->query("Select * FROM playerranksdb ORDER BY PVPKills DESC LIMIT 0, :limit",(Array('limit' => $limit)));
			else
				$GetPrice = $this->db->query("Select * FROM playerranksdb ORDER BY PVPKills DESC",(Array()));
		...
		public function GetDeath($id){
			$curl_handle=curl_init();
			curl_setopt($curl_handle, CURLOPT_URL, self::SERVER_API_ADDRESS.':'.self::SERVER_API_PORT.'/deaths.json');
			curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
			curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($curl_handle, CURLOPT_ENCODING , "gzip");
			$query = curl_exec($curl_handle);
